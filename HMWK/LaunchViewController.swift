            ...
            if success {
                ...
                for course in
                    FirebaseData.data.currentUser!.enrolledCourses {
                        ReadFirebaseData.readCourse(courseId: course.courseID, completion: {(success) in
                            
                            if success {
                               // FirebaseData.data.promptsInEnrolledCourses?.append(contentsOf: course.coursePrompts)
                                
                                for prompt in course.coursePrompts {
                                    ReadFirebaseData.readResponses(courseId: course.courseID, promptId: prompt.promptID, completion: {(success) in
